...
// PACS0001 — keyed by CONST.UNITS type; a missing type falls back to worker cadence (also gaitEase/idlePose/drawUnit/drawCombatFx) — AGENTS.md (known sites, not exhaustive)
const VOLLEY = {
...
// PACS0008 — must match engine core attacker id "<side>_core" or core-attack FX silently vanish — AGENTS.md (known sites, not exhaustive)
...
function startJob(log){
  ...
  (log.buildsCompleted||[]).forEach(b=>{
    // PACS0006 — recovers side from id char0 (build-spawn log has no side field); depends on engine mintId format — AGENTS.md (known sites, not exhaustive)
...
function drawUnit(g_, u, t){
  ...
  // PACS0017 — every CONST.UNITS type needs a "<type>_default" sprite in atlas.js or the unit draws blank — AGENTS.md (known sites, not exhaustive)
