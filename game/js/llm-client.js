...
(function (g) {
  ...
  function buildSystemPrompt() {
    ...
    cachedPrompt = [
      ...
      '- Core A anchored at [' + C.CORE_POS.A.join(',') + '], Core B at [' + C.CORE_POS.B.join(',') + ']. Each Core occupies a 2x2 block (the anchor plus the three tiles toward its corner); all 4 tiles are permanently blocked.',
      '- ALL distances and ranges are Chebyshev: dist = max(|dx|,|dy|). Diagonals count as 1. Range to/from a Core is measured to its NEAREST occupied tile.',
      '- Tiles are EXCLUSIVE AT REST: one unit per tile, and Core tiles are never enterable. Units pass THROUGH occupied tiles while moving; only the tile a unit STOPS on must be free.',
      '',
      'MATCH FLOW',
      ...
      '- Win instantly by destroying the enemy Core. At turn ' + C.TURN_LIMIT + ' (if reached): higher Core HP% wins; tie broken by total remaining unit HP; if still equal it is a DRAW (a real outcome, not a loss).',
      '- STAGNATION (coward tax): after turn ' + C.STAGNATION.afterTurn + ' and before turn ' + C.MELTDOWN.fromTurn + ', any full turn with ZERO combat costs BOTH Cores ' + C.STAGNATION.dmg + ' HP. Do not sit back; force an engagement.',
      '- DATACENTER MELTDOWN: from turn ' + C.MELTDOWN.fromTurn + ' to turn ' + C.TURN_LIMIT + ', BOTH Cores lose ' + C.MELTDOWN.dmg + ' HP EVERY turn, UNCONDITIONALLY — fought or not. That drains a full Core in ten turns, so the match cannot reach turn ' + C.TURN_LIMIT + ' with both Cores alive: win before turn ' + C.MELTDOWN.fromTurn + ', or make sure your Core out-HPs the enemy\'s when the burn finishes you both.',
      '',
      ...
      '- Units pass through occupied tiles in transit; only the LANDING tile must be free. If your destination is occupied, the unit is redirected to the nearest free tile beside it (from the mover\'s side). Blocked and out-of-range both count as clamped, never failed.',
